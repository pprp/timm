...
CUDA_VISIBLE_DEVICES=0,1,2 bash ./distributed_train.sh 3 42231 $DATA \
    --model mp_mobilenet_v2_075 \
    -b 512 -j 4 \
    --sched cosine --epochs 200 \
    --decay-epochs 1.6 --decay-rate .973 \
    --opt lamb --opt-eps 1e-06 \
    --warmup-lr 1e-6 --weight-decay 0.09 --warmup-epochs 5 \
    --drop 0.1 --drop-path 0.08 \
    --aa rand-m9-mstd0.5 --remode pixel \
    --reprob 0.2 --amp \
    --lr 0.00375 --lr-noise 0.42 0.9 \
    --interpolation bicubic --min-lr 1e-5 --mixup 0.15 \
    --num-classes 1000 \
    --output metapooling_with_procedureB_075 \
    --seed 42 \
    --train-interpolation bicubic