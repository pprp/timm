...
# class SpatialSeperablePooling_v1(nn.Module):
...
class MetaPooling(nn.Module):
    def __init__(self, in_channels=128, ratio: float = 0.5):
        ...
        self.spatial_sep_pool = SpatialSeperablePooling(in_channels)
        self.split = SplitBlock(ratio=0.5)
        self.bn = nn.BatchNorm2d(in_channels)
        self.act = h_swish()

    def forward(self, x):
        x1, x2 = self.split(x)
        out = self.spatial_sep_pool(x2)
        out = self.bn(out)
        out = self.act(out)
...
class InvertedResidual(nn.Module):
    def __init__(self, in_channels, out_channels, stride, expand_ratio, idx=6):
        ...
        self.meta = SpatialSeperablePooling(out_channels) if idx >= 6 else nn.Identity()
